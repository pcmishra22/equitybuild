...
function LoadCalendarScript(callback){
	function LoadFullCalendarScript(){
		if(!$.fn.fullCalendar){
			$.getScript(baseurl+'plugins/fullcalendar/fullcalendar.js', callback);
		}
		else {
	...
	if (!$.fn.moment){
		$.getScript(baseurl+'plugins/moment/moment.min.js', LoadFullCalendarScript);
	}
	else {
...
function LoadTimePickerScript(callback){
	if (!$.fn.timepicker){
		$.getScript(baseurl+'plugins/jquery-ui-timepicker-addon/jquery-ui-timepicker-addon.min.js', callback);
	}
	else {
		if (callback && typeof(callback) === "function") {
			callback();
...
function LoadBootstrapValidatorScript(callback){
	if (!$.fn.bootstrapValidator){
		$.getScript(baseurl+'plugins/bootstrapvalidator/bootstrapValidator.min.js', callback);
	}
	else {
		if (callback && typeof(callback) === "function") {
			callback();
...
function LoadSelect2Script(callback){
	if (!$.fn.select2){
		$.getScript(baseurl+'plugins/select2/select2.min.js', callback);
	}
	else {
		if (callback && typeof(callback) === "function") {
			callback();
...
function LoadDataTablesScripts(callback){
	function LoadDatatables(){
		$.getScript(baseurl+'plugins/datatables/jquery.dataTables.js', function(){
			$.getScript(baseurl+'plugins/datatables/ZeroClipboard.js', function(){
				$.getScript(baseurl+'plugins/datatables/TableTools.js', function(){
					$.getScript(baseurl+'plugins/datatables/dataTables.bootstrap.js', callback);
				});
			});
		});
	}
...
function LoadFineUploader(callback){
	if (!$.fn.fineuploader){
		$.getScript(baseurl+'plugins/fineuploader/jquery.fineuploader-5.0.1.min.js', callback);
	}
	else {
		if (callback && typeof(callback) === "function") {
			callback();
...
function LoadXChartScript(callback){
	function LoadXChart(){
		$.getScript(baseurl+'plugins/xcharts/xcharts.min.js', callback);
	}
	function LoadD3Script(){
		if (!$.fn.d3){
			$.getScript(baseurl+'plugins/d3/d3.v3.min.js', LoadXChart)
		}
		else {
			LoadXChart();
		}
...
function LoadFlotScripts(callback){
	function LoadFlotScript(){
		$.getScript(baseurl+'plugins/flot/jquery.flot.js', LoadFlotResizeScript);
	}
	function LoadFlotResizeScript(){
		$.getScript(baseurl+'plugins/flot/jquery.flot.resize.js', LoadFlotTimeScript);
	}
	function LoadFlotTimeScript(){
		$.getScript(baseurl+'plugins/flot/jquery.flot.time.js', callback);
	}
	if (!$.fn.flot){
		LoadFlotScript();
	}
...
function LoadMorrisScripts(callback){
	function LoadMorrisScript(){
		if(!$.fn.Morris){
			$.getScript(baseurl+'plugins/morris/morris.min.js', callback);
		}
		else {
			if (callback && typeof(callback) === "function") {
				callback();
	...
	if (!$.fn.raphael){
		$.getScript(baseurl+'plugins/raphael/raphael-min.js', LoadMorrisScript);
	}
	else {
		LoadMorrisScript();
	}
...
function LoadFancyboxScript(callback){
	if (!$.fn.fancybox){
		$.getScript(baseurl+'plugins/fancybox/jquery.fancybox.js', callback);
	}
	else {
		if (callback && typeof(callback) === "function") {
			callback();
...
function LoadKnobScripts(callback){
	if(!$.fn.knob){
		$.getScript(baseurl+'plugins/jQuery-Knob/jquery.knob.js', callback);
	}
	else {
		if (callback && typeof(callback) === "function") {
			callback();
...
function LoadSparkLineScript(callback){
	if(!$.fn.sparkline){
		$.getScript(baseurl+'plugins/sparkline/jquery.sparkline.min.js', callback);
	}
	else {
		if (callback && typeof(callback) === "function") {
			callback();
...
---------------------------------------------*/
//
//save event to save event to database
//
function saveEventToDatabase(title,description,start,end)
{
	if(title !='' && start!='')
	{	
	...
	//edt=start.getFullYear()+'-'+(start.getMonth()+1)+'-'+start.getDate();
	//etm=start.getHours()+':'+start.getMinutes()+':'+start.getSeconds();

	$.ajax
		({
			url: baseurl+'admin/saveevent',
			type: "POST",
			data: "title="+title+"&desc="+description+"&start="+start+"&end="+end,
			success: function(data) 
			{
				$('#eventaddmessage').html(data);
			},
...
//
//update event to save event to database
//
function updateEventToDatabase(id,title,description,start,end)
{	
	if(title !='')
	{	
	$.ajax
		({
			...
			type: "POST",
			data: "title="+title+"&desc="+description+"&id="+id+"&start="+start+"&end="+end,
			success: function(data) 
			{
				$('#eventaddmessage').html(data);
			},
...
function TestTable3(){
	$('#datatable-3').dataTable( {
		...
		"oTableTools": {
			"sSwfPath": baseurl+"plugins/datatables/copy_csv_xls_pdf.swf",
			"aButtons": [
				"copy",
				"print",
				{
...
function FileUpload(){
	$('#bootstrapped-fine-uploader').fineUploader({
		...
		thumbnails: {
			placeholders: {
				waitingPath: baseurl+"assets/waiting-generic.png",
				notAvailablePath: baseurl+"assets/not_available-generic.png"
			}
		},
		request: {
			endpoint: baseurl+'server/handleUploads'
		},
		validation: {
...
/*					alert(new_event_name);
					alert( $('#newevent_desc').val());
					alert(start);
					alert(end);*/
					
					//return false;
					//save event to database	
					saveEventToDatabase(new_event_name, $('#newevent_desc').val(),start,end);
					//save event to database
		...
		eventResize: function(event, delta, revertFunc) {
			//update event	
			updateEventToDatabase(event.id,event.title,event.description,event.start,event.end);
    	},
		eventClick: function(calEvent, jsEvent, view) {
			...
			$('#event_change').on('click', function(){
				...
				//update event
				updateEventToDatabase(calEvent.id,calEvent.title,calEvent.description,calEvent.start,calEvent.end);
				//update event
